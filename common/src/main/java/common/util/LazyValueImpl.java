...
public class LazyValueImpl<T> implements LazyValue<T> {
    ...
    static {
        try {
            THE_LOCK =
                MethodHandles.lookup().findVarHandle(LazyValueImpl.class, "theLock", Semaphore.class);
    ...
    /**
     * Ensure only a single thread calls the delegate if the value is not yet loaded. Note that {@code
     * loadedCopy} and {@code theLockCopy} represent thread copies of the corresponding volatile
     * variables, while {@code LOADED} and {@code THE_LOCK} are var references to those volatile
     * variables.
     *
     * @return the value
     */
